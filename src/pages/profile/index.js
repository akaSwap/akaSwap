...
import { getLanguage, ManagerAddr, ENV } from '../../constants'
import { VisuallyHidden } from '../../components/visually-hidden'
import { Creation, Collection, GachaAndBundle, Auction, Record } from './tabs'
import { GetUserMetadata } from '../../data/api'
import styles from './styles.module.scss'
import ValidatedIcon from '../../imgs/validated.svg'
import ManagerIcon from '../../imgs/manager.svg'
...
export default class Profile extends Component {
  ...
  state = {
    ...
    walletPrev: showWallet({ wallet: window.location.pathname.split('/')[2] }),
    validated: -1,
    render: false,
    loading: true,
    akaDao: 0,
  ...
  componentDidMount = async () => {
    ...
    await Promise.all([
      GetUserMetadata(this.state.wallet).then((data) => {
        if (data.data.alias) {
          this.setState({ alias: data.data.alias })
          axios.post(`${process.env.REACT_APP_INTERNAL_ACCOUNTS}/${this.state.wallet}/metadata`).catch(e => e)
        }
        ...
        if (data.data.facebook) this.setState({ facebook: data.data.facebook })
        if (data.data.email) this.setState({ email: data.data.email })
      }),
      axios.get(process.env.REACT_APP_FEATUREDLIST_WALLET)
        .then((res) => {
          if (res.data.indexOf(this.state.wallet) === 0) {
            this.setState({ validated: 0 })
          }
          else if (res.data.indexOf(this.state.wallet) > 0) {
            this.setState({ validated: 1 })
          }
        })
        .catch(),
      axios
        .get(`${process.env.REACT_APP_ACCOUNTS}/${this.state.wallet}/akadao`)
        .then((res) => {
          this.setState({
            akaDao: res.data.akaDao === undefined ? 0 : res.data.akaDao,
            drawIdenticon: true
          })
        })
        .catch()
  ...
  tabNames = [
    this.language.mypage.creations,
    this.language.mypage.collection,
    this.language.mypage.gachaAndBundle,
    this.language.mypage.auction,
    this.language.mypage.records.records
  ]
  ...
  render() {
    ...
    let walletUrl = ''
    if (ENV === 'dev')
      walletUrl = `http://128.199.70.49:8001/granadanet/${this.state.wallet}/operations`
    else if (ENV === 'test')
      walletUrl = `https://better-call.dev/florencenet/${this.state.wallet}/operations`
    else
      walletUrl = `https://better-call.dev/mainnet/${this.state.wallet}/operations`
    return (
      <Page title={this.state.wallet} noBottomPadding>
        <Container>
          <Padding>
            <div className={styles.profile}>
              {this.state.drawIdenticon &&
              ...
              <div className={styles.info}>
                ...
                <div className={styles.link}>
                  {this.language.mypage.wallet}:{' '}
                  <Button
                    href={walletUrl}
                  >
                    {this.state.walletPrev}
                  </Button>
                  {ManagerAddr.indexOf(this.state.wallet) >= 0 &&
                    <span className={styles.validated}>
                      <img src={ManagerIcon} alt="Validated" />
                      <div className={styles.hoverMessage}>
                        {this.language.mypage.manager}
                      </div>
                    </span>
                  }
                  {this.state.validated > 0 &&
                    <span className={styles.validated}>
                      <img src={ValidatedIcon} alt="Validated" />
                      <div className={styles.hoverMessage}>
                        {this.language.mypage.verified}
                      </div>
                    </span>
                  }
                </div>
                ...
                <p>
                  {this.language.mypage.myAkaDao}:{' '}
                  {/* {Number.parseFloat(this.state.akaDao / 1000000).toFixed(6)}{' '} */}
                  {Number.parseFloat(this.state.akaDao / 1000000)}{' '}
                  <b>⩘</b>
                </p>
                ...
                <div>
                  ...
                  {this.state.email && (
                    <Button href={"mailto:" + this.state.email}>
                      <VisuallyHidden>{this.state.email}</VisuallyHidden>
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="16"
                        height="16"
                  ...
                  {this.state.facebook && (
                    ...
                    >
                      <VisuallyHidden>{`https://facebook.com/${this.state.facebook}`}</VisuallyHidden>
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="18"
                        height="18"
          ...
          <Padding>
            <div className={styles.display}>
              <Container full>
                <Padding>
                  <TabBar
                    className={((this.context.address !== undefined && this.context.address.address === this.state.wallet) ? styles.scroll : '') + ' ' + styles.menu}
                    current={this.state.tabIndex}
                    tabNames={(this.context.address !== undefined && this.context.address.address === this.state.wallet) ? this.tabNames : this.tabNamesOther}
                    onClickCB={(i) => this.setTabIndex(i)}
                  />
              ...
              {this.context.address !== undefined &&
                this.context.address.address === this.state.wallet &&
